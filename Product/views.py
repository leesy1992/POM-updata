...
from Autotest_platform.common import *
...
#后端设置分页数量
def slicing(value, index=1, size=10):
...
#Django分页
class Fenye():
    def paginator(value,index=1,page_size=10):
        """分页显示"""
    ...
    """计算页码"""       
    def num_pages(value,page_size=10):
        return   Paginator(value,page_size).num_pages     
...
class Project:
    ...
    def find(request):
        ...
        pages=Fenye.num_pages(projects)
        projects=Fenye.paginator(projects,page_index,page_size) 
        project_list = list()
        for p in projects:
...
class Environment:
    ...
    def find(request):
        ...
        try:
            ...
            return JsonResponse(400, "时间参数错误")
        pages=Fenye.num_pages(environments)
        environments=Fenye.paginator(environments,page_index,page_size)
...
class Page:
    ...
    def find(request):
        ...
        try:
            ...
            return JsonResponse(400, "时间参数错误")
        page_num=Fenye.num_pages(pages)
        pages=Fenye.paginator(pages,page_index,page_size)
        page_list = list()
        for p in pages:
...
class Element:
    ...
    def find(request):
        ...
        try:
            ...
            return JsonResponse(400, "时间参数错误")
        pages=Fenye.num_pages(elements)
        elements=Fenye.paginator(elements,page_index,page_size)
        element_list = list()
        for e in elements:
...
class Keyword:
    ...
    def find(request):
        ...
        try:
            ...
            return JsonResponse(400, "时间参数错误")
        pages=Fenye.num_pages(ks)
        ks=Fenye.paginator(ks,page_index,page_size)
...
class TestCase:
    ...
    def find(request):
        ...
        try:
            ...
            return JsonResponse(400, "时间参数错误")
        pages=Fenye.num_pages(tcs)
        tcs=Fenye.paginator(tcs,page_index,page_size)            
        tcs_list = list()
        for tc in tcs:
            dic = model_to_dict(tc, ["id", "projectId", 'title', 'level', 'remark',  'selectText'])
...
class TestResult:
    ...
    def find(request):
        ...
        page_size = parameter.get("pageSize", 10)
        # page_size = int(page_size) if str(page_size).isdigit() and int(page_size) >= 1 else 10
        start_time = "2022-01-01 00:00:00" #设定时间，减少测试数据
        start_time = parameter.get("startTime", start_time) if parameter.get("startTime", start_time) else start_time
        now = timezone.now().strftime('%Y-%m-%d %H:%M:%S')
        end_time = parameter.get("endTime", now) if parameter.get("endTime", now) else now
        try:
            ...
            return JsonResponse(400, "时间参数错误")
        pages=Fenye.num_pages(res)
        res=Fenye.paginator(res,page_index,page_size)
        tcs_list = list()
        tcs_li=dict()
        tass =list(get_model(project,get=False).values('id','name'))
        for ta in tass :
            tcs_li[ta['id']]=ta['name']
        for re in res:
            dic = model_to_dict(re, ["id", "projectId", "taskId", "testcaseId", 'title', 'status' ])
            ...
            dic["projectName"] = tcs_li[re.projectId] if re.projectId else ""
    ...
    def get(request, result_id):
        ...
        splitResult = list()
        for s in split:
            sd = model_to_dict(s, ["status", 'expect', 'remark','loadtime'])
...
class Public:
    ...
    def index(request):
        ...
        for p in projects:
            ...
            st = Result.objects.filter(projectId=p.id, status=30,createTime__gt=start_time)
            info["successfulTotal"] = len(st)
            ft = Result.objects.filter(projectId=p.id,createTime__gt=start_time).exclude(status=30)
    ...
    def bar_char(request):
        ...
        for p in all_project:
            ...
            all_result = Result.objects.filter(projectId=p.id,createTime__gt=start_time)
...
class TestTasks:
    ...
    def create(request):
        ...
        t.crontab = parameter.get("crontab", "")
        ...
        try:
            t.save()
            kwargs={"name":t.name,"crontab":t.crontab,"timing":t.timing}
            task_login(**kwargs)
    ...
    def find(request):
        ...
        try:
            ...
            return JsonResponse(400, "时间参数错误")
        pages=Fenye.num_pages(ts)
        ts=Fenye.paginator(ts,page_index,page_size)
        ts_list = list()
        for t in ts:
            dic = model_to_dict(t, ["id", 'name', 'crontab', 'timing'])
    ...
    def get(request, task_id):
        ...
        if not t:
            return JsonResponse.BadRequest("该任务不存在")
        result = model_to_dict(t, ["id", 'name', 'crontab', 'timing'])
...
class Login:
    ...
    def find(request):
        ...
        try:
            ...
            return JsonResponse(400, "时间参数错误")
        pages=Fenye.num_pages(ls)
        ls=Fenye.paginator(ls,page_index,page_size)
        ls_list = list()
        for l in ls:
