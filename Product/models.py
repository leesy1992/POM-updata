    ...
    def buid(self):
        ...
        if browser != 'android':
            ...
            if browser == 'chrome':
                ...
                from selenium.webdriver.common.desired_capabilities import DesiredCapabilities
                browser = webdriver.Remote(command_executor="{}/wd/hub".format(browser),desired_capabilities=DesiredCapabilities.CHROME.copy())
            ...
            desired_caps = {
                ...
                'deviceName': self.driverPath,#设备名称。如果是真机，在'设置->关于手机->设备名称'里查看

                'appPackage': 'com.irenshi.personneltreasure',# apk的包名

                'appActivity': 'com.irenshi.personneltreasure.activity.WelcomeActivity',# activity 名称

                "noReset": True,#是否清除session信息

                "noSign": False

            }
            browser = webdriver.Remote(self.installPath, desired_caps)   # 连接Appium
            time.sleep(2)
            return browser
...
class Count(models.Model):
    ...
    counts = models.IntegerField(null=False, default=1)
...
class tasktime(models.Model):
    title = models.CharField(max_length=255, null=False)
    last_title=models.IntegerField(null=True, default=0)
    single_time = models.CharField(max_length=255, null=False)
    createTime = models.DateTimeField(default=timezone.now)


class last_title(models.Model):
