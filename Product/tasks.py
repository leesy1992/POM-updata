...
def SplitTaskRunning(splitResult_id):
    ...
    from Autotest_platform.common import get_model,get_timing,send_dingding,send_email
    ...
    for stp in steps:
        try:
            keyname = Keyword.objects.get(id=stp.get("keywordId")).name        
            if stp.get("values"):
                    elemetname = Element.objects.get(id=stp.get("values")[0]["value"]).name
            else:
                    elemetname=""     
        except:
                elemetname=stp.get("values")[0]["value"]   
        ls=[keyname,elemetname]
        loglist.append(ls)
    for step in steps:
        try:
            #执行测试步骤（核心）
            ...
            log.info(durtions) 
            split.remark="第" + str(index) + "步:"+loglist[index-1][0] + loglist[index-1][1] #操作步骤的log  
            #耗时计算显示逻辑
            try:
                if  index==1:
                    getTime=get_timing()
                    ...
                    log.info(getTime) 
                elif "断言" in loglist[index-1][0]:
                    if split.loadtime:
                        split.loadtime=str(split.loadtime) +"/" + str(durtions) #计算步骤的加载时间并存入数据库                    
                    else:
                        split.loadtime=durtions

                else:                      
                  if split.loadtime:
                        split.loadtime=str(split.loadtime) +"/" + str(" ") #计算步骤的加载时间并存入数据库
                  else:
                        split.loadtime=" "
            except Exception as e:
...
def timingRunning(**kwargs):    
    ...
    from Autotest_platform.common import get_model
