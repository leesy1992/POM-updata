...
from Autotest_platform.common import get_time
...
class PageObject:
    ...
    def click(self, locator):
        if locator is None:
            ...
            # try:
                elemnt=self.find_element(self.driver, locator).click()
            # except :
    ...
    @get_time
    def assert_exist(self,locator):
          try:
              self.find_element(self.driver,locator)
              return   self.get_text(locator) 
          except:
              raise AssertionError('断言不通过,目标元素不存在')
